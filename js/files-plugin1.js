...
document.addEventListener('click', (e) => {
	...
	renderCombinedMetadataForm(fileFields, groupfolderFields) {
		...
		if (isGroupfolderRoot && groupfolderFields && groupfolderFields.length > 0) {
			formHtml += '<div class="metavox-sidebar-section">' +
				'<h3>Team folder Metadata</h3>' +
				'<div class="metavox-field-display" style="background: var(--color-background-dark);">' +
			...
			formHtml += '<div class="metavox-field-display" style="background: var(--color-primary-light);">' +
					'<span class="metavox-field-label">Admin Hint</span>' +
					'<span class="metavox-field-value">' +
						'To edit this Team folder metadata, go to Admin Settings → MetaVox → Team folders' +
					'</span>' +
				'</div>' +
			'</div>';
		}
	...
	formatFieldValue(field) {
		...
		// Handle multiselect values - they are stored as comma-separated strings
		if (field.field_type === 'multiselect' && value && value !== '(Not set)') {
			// Split comma-separated values and join with a more readable format
