...
export default {
  ...
  data() {
    return {
      ...
      // CACHE properties - 15 minuten cache
  ...
  methods: {
    ...
    async loadFields() {
      ...
      try {
        ...
        // Filter voor team folder metadata fields (applies_to_groupfolder = 1)
        const fields = (response.data || []).filter(field => 
          field.applies_to_groupfolder === 1 || 
          field.applies_to_groupfolder === '1'
        )
    ...
    // Gecachte loadFieldUsageCountsInBackground methode:
    ...
    // Helper methode om cached usage counts toe te passen:
    ...
    // Methode om cache te invalideren:
    ...
    getUsageCount(field) {
      // Gebruik altijd usage_count, met 0 als fallback
      return field.usage_count !== undefined ? field.usage_count : 0
    },
