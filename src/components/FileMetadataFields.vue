...
async loadFieldUsageCountsInBackground() {
  try {
    ...
    console.log('Usage counts loaded and cached for 5 minutes')
...
async addField() {
  ...
  if (finalFieldName && !finalFieldName.startsWith('file_')) {
    finalFieldName = 'gd_file_' + finalFieldName
  }
  ...
  try {
    ...
    for (const fieldData of this.importPreviewData) {
      try {
        ...
        // Add gf_file_ prefix if missing
        if (importData.field_name && !importData.field_name.startsWith('gf_file_')) {
          importData.field_name = 'gf_file_' + importData.field_name
        }
